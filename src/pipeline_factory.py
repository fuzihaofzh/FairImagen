...
# Optional model-class imports to avoid import-inside-function warnings
try:
    from diffusers import PixArtSigmaPipeline
except ImportError:  # Older diffusers versions
    PixArtSigmaPipeline = None  # type: ignore[assignment]

try:
    from diffusers import WuerstchenDecoderPipeline
except ImportError:
    WuerstchenDecoderPipeline = None  # type: ignore[assignment]
...
def create_pipeline(pipename="sd3.0", **kwargs):
    ...
    # PixArt-Σ (DiT architecture, like SD3)
        ...
        try:
            if PixArtSigmaPipeline is None:
                raise ImportError
            ...
            msg = "PixArt-Sigma requires latest diffusers"
            raise ValueError(msg) from e

    # Würstchen v3 (efficient cascade model)
    elif pipename == "wurstchen":
        try:
            if WuerstchenDecoderPipeline is None:
                raise ImportError
            ...
            msg = "Würstchen requires latest diffusers"
            raise ValueError(msg) from e

    else:
        msg = f"Unknown pipeline: {pipename}"
...
def get_pipeline_info(pipename):
    ...
    return info.get(
        pipename,
        {"type": "unknown", "arch": "unknown", "params": "unknown"},
    )
